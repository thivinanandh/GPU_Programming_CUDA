...
__global__ void BitonicMerge(int size, unsigned int* arrayDevice,int numStages,int N_ThreadCycle)
{
    ...
    __shared__ int sharedDeviceArray_1[2048];
    ...
    for( int stage =  1 ;  stage < numStages ; stage++)
    {
       ...
       // if(ThreadId == 0 ) printf("@@@@@@@-------STAGE %d -----@@- \n",stage);
        ...
        //    if(ThreadId == 0 ) printf(" -------------STAGE %d --- SUBSTAGE %d-------------------- \n",stage,substage);
            unsigned int Displace =  1 << substage - 1 ;         // NUmber of elements to Be Displaced 
            // cout << " displace : " << Displace <<endl; 
            if(N_ThreadCycle == 0) N_ThreadCycle++;
            for (unsigned int ThreadCycle = 0 ; ThreadCycle < N_ThreadCycle  ; ThreadCycle++)
            {
                //COPY  THE ARRAYS  to Shared memory
                unsigned int ThreadId_new = ThreadId + ThreadCycle*blockDim.x;   
                ...
                if (ThreadId_new < size)
                {
                    ...
                    {
                        sharedDeviceArray_1[ThreadId       ] = arrayDevice[ThreadId_new];
                        sharedDeviceArray_1[ThreadId + 1024] = arrayDevice[ThreadId_new + Displace];
                        if(  ( ThreadId_new & (1 << (stage)) )   )   // N+1th bit Set as 1 , So Decending
                        {
                            ...
                            if(sharedDeviceArray_1[ThreadId       ] < sharedDeviceArray_1[ThreadId + 1024])
                            {
                                arrayDevice[ThreadId_new]               = sharedDeviceArray_1[ThreadId + 1024];
                                arrayDevice[ThreadId_new + Displace]    = sharedDeviceArray_1[ThreadId       ];
                            }
                        }
...
__global__ void BitonicSort(int size, unsigned int* arrayDevice,int numStages,int N_ThreadCycle)
{
    ...
    __shared__ int sharedDeviceArray_1[2048];
   // if(ThreadId == 0)   printf("NUmStages : %d , NTCycle : " , numStages,N_ThreadCycle);

    for ( int stage = numStages ;  stage > 0 ; stage--)
    {
        //Pick NUmbers which has 'stage'th bit set as 0
        unsigned int Displace = 1 << (stage-1)  ;
        if(N_ThreadCycle == 0) N_ThreadCycle++; 
        for (unsigned int ThreadCycle = 0 ; ThreadCycle < N_ThreadCycle  ; ThreadCycle++)
        {
            unsigned int ThreadId_new = ThreadId + ThreadCycle*blockDim.x;
